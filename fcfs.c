...
main()
{
    ...
    atat = (float)ttat / n;
    tp = n / (float)tbt;
	cut = (((float)tbt + ci)/(float)tbt)*100;
    ...
    printf("\nThroughput = n / tbt  = %d/%d = %5.2f 1/ms",n,tbt,tp);
    printf("\nCPU Utililazation=((CPU busy time + CPU idel time)/CPU busy time)*100= ((%d+%d)/%d)*100 = %3.2f percentage",tbt,ci,tbt,cut);
}
